...
#define MINEUTILS_MAJOR_VERSION "3"              // 主版本号，对应不向下兼容的API或文件改动
#define MINEUTILS_MINOR_VERSION "0"              // 次版本号，对应不影响现有API使用的新功能增加
#define MINEUTILS_PATCH_VERSION "0"              // 修订版本号，对应不改变API的BUG修复或效能优化
#define MINEUTILS_DATE_VERSION "20251219-alpha"  // 日期版本号，对应文档和注释级别的改动和测试阶段
...
#define _MINE_NOREMOVE volatile
    ...
    inline T _safeAdd(T a, T b, const char* func_name)
    {
        if (b > 0 && a > std::numeric_limits<T>::max() - b)
        {
            mprintfW("Addition overflow detected in %s! Returned max value\n", func_name);
            return std::numeric_limits<T>::max();  // 正溢出
        }

        if (b < 0 && a < std::numeric_limits<T>::min() - b)
        {
            mprintfW("Addition underflow detected in %s! Returned min value\n", func_name);
    ...
    template <class T, typename std::enable_if<std::is_integral<T>::value, int>::type = 0>
    inline T _safeSub(T a, T b, const char* func_name)
    {
        if (b > 0 && a < std::numeric_limits<T>::min() + b)
        {
            mprintfW("Subtraction underflow detected in %s! Returned min value\n", func_name);
            return std::numeric_limits<T>::min();  // 负溢出
        }
        if (b < 0 && a > std::numeric_limits<T>::max() + b)
        {
            mprintfW("Subtraction overflow detected in %s! Returned max value\n", func_name);
            return std::numeric_limits<T>::max();  // 正溢出
        }
        return a - b;  // 不会溢出
    }

...
namespace mthrd
{
    ...
    // 标准库mutex通常自带自旋锁优化，因此在大多数情况下并不推荐使用SpinLock
    ...
    public:
        ...
        /*  使用RAII方式对局部区域加锁
            ...
            @return 一个私有类RGuard对象，只能用auto推导；构造时加锁，调用reset函数或析构时解锁  */
    ...
    public:
        ...
        /*  使用RAII方式对局部区域加写锁
            ...
            @return 一个私有类WGuard对象，只能用auto推导；构造时加锁，调用reset函数或析构时解锁  */
    ...
    public:
        ...
        template <class Fn, class... Args, class Ret = typename mtype::StrictStdBindTraits<Fn, Args...>::ReturnType, typename std::enable_if<std::is_same<Ret, typename mtype::StrictStdBindTraits<Fn, Args...>::ReturnType>::value && !std::is_reference<Ret>::value && (std::is_void<Ret>::value || std::is_copy_constructible<Ret>::value), int>::type = 0>
...
namespace mthrd
{
    ...
    class ReadWriteMutex::WGuard final
    {
    public:
        // 只允许移动构造，不允许移动赋值和拷贝
    ...
    inline void TaskFuture<Ret>::wait() const
    {
        if (future_state_.valid())
            ...
            mprintfW("Task is invalid, so the function returns directly!\n");
    }
    ...
    inline ThreadPool::ThreadPool(uint32_t pool_size)
    {
        ...
        work_thds_.resize(pool_size);
        for (uint32_t i = 0; i < pool_size; ++i)
        {
            work_thds_[i] = std::thread(&ThreadPool::worker, this);
        }
    }
    ...
    template <class Fn, class... Args, class Ret, typename std::enable_if<std::is_same<Ret, typename mtype::StrictStdBindTraits<Fn, Args...>::ReturnType>::value && !std::is_reference<Ret>::value && (std::is_void<Ret>::value || std::is_copy_constructible<Ret>::value), int>::type>
    ...
    {
        if (need_pause_.load(std::memory_order_relaxed))
        {
            std::unique_lock<std::mutex> lk(mtx_);
            while (need_pause_.load(std::memory_order_relaxed))
            {
                splk_.lock();
                state_map_[point_id] = true;
                splk_.unlock();
                cond_.wait(lk);
            }
        }
        splk_.lock();
        state_map_[point_id] = false;
        splk_.unlock();
    }

    inline void ThreadPauser::pause()
    {
        need_pause_.store(true, std::memory_order_relaxed);
    }
    ...
    inline void ThreadPauser::resume()
    {
        {
            std::lock_guard<std::mutex> lk(mtx_);
            need_pause_.store(false, std::memory_order_relaxed);
        }
        cond_.notify_all();  // 据说notify内部，先加锁cond_var内部锁，再通知，因此不用担心cond_var在lk解锁后进入wait前，notify触发而导致错过
    }

    inline bool ThreadPauser::isPaused(uint8_t point_id)
    {
        {
            auto lk_guard = splk_.lockGuard();
            auto it = state_map_.find(point_id);
...
namespace _mthreadcheck
{
    inline void SpinLockTest()
    {
        ...
        if (!(func1_check_ret && func2_check_ret))
            mprintfE(R"(Failed when check SpinLock)"
                     "\n");
    }
    ...
    inline void ReadWriteMutexTest()
    {
        ...
        };
        {
            ...
            if (func1_check_ret && func2_check_ret)
                mprintfW(R"(Maybe failed this time when check: ReadWriteMutexTest Read-Read Lock. Try again!)"
                         "\n");
        }
...

        {
            ...
            if (!(func1_check_ret && func4_check_ret))
                mprintfE(R"(Failed when check: ReadWriteMutexTest Read-Write Lock)"
                         "\n");
        }

        {
            ...
            thd3.join();
            thd4.join();
            if (!(func3_check_ret && func4_check_ret))
                mprintfE(R"(Failed when check: ReadWriteMutexTest Write-Write Lock)"
                         "\n");
        }
    }
    ...
    inline void ThreadPoolTest()
    {
        ...
        mthrd::TaskFuture<void> state1 = thread_pool.addTask([](int x)
                                                             { std::this_thread::sleep_for(std::chrono::seconds(1)); }, 1);
        auto ft = state1.toFuture();
        ft.wait();
        if (state1.get())
            mprintfE(R"(Failed when check: thread_pool.addTask 1)"
                     "\n");

        mthrd::TaskFuture<int> state2 = thread_pool.addTask([](int x)
                                                            {std::this_thread::sleep_for(std::chrono::seconds(1)); return x; }, 1);
        if (*state2.get() != 1)
            mprintfE(R"(Failed when check: thread_pool.addTask 2)"
                     "\n");
    }

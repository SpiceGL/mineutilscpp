    ...
    inline std::vector<std::string> _walk(const std::string& path, bool return_path)
    {
        ...
        for (std::string& f_d_path : listdir_res)
        {
            if (mpath::_isDir(f_d_path))
            {
                ...
                return_path ? filenames.push_back(f_d_path) : filenames.push_back(mpath::_splitName(f_d_path, true));
        }
        return filenames;
    }
...
namespace _mpathcheck
{
    inline void mpathTest()
    {
        ...
        if (!ret)
            mprintfE(R"(Failed when check: mpath::normPath("./abc"):%s == "abc")"
                     "\n",
        ...
        if (!ret)
            mprintfE(R"(Failed when check: mpath::splitName("./abc/file.txt", true):%s == "file.txt")"
                     "\n",
                     mpath::splitName("./abc/file.txt", true).c_str());
        ret = mpath::splitName("./abc/file.txt", false) == "file";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::splitName("./abc/file.txt", false):%s == "file")"
                     "\n",
                     mpath::splitName("./abc/file.txt", false).c_str());

        ret = mpath::splitExt("./abc/file.txt") == "txt";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::splitExt("./abc/file.txt"):%s == "txt")"
                     "\n",
                     mpath::splitExt("./abc/file.txt").c_str());
        ret = mpath::splitExt("txt") == "";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::splitExt("txt"):%s == "")"
                     "\n",
                     mpath::splitExt("txt").c_str());
        ret = mpath::splitExt("abc.aa/txt") == "";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::splitExt("abc.aa/txt"):%s == "")"
                     "\n",
                     mpath::splitExt("abc.aa/txt").c_str());

        ret = mpath::isAbs("/aa/bb/cc");
        if (!ret)
            mprintfE(R"(Failed when check: mpath::isAbs("/aa/bb/cc"):%d)"
                     "\n",
                     mpath::isAbs("/aa/bb/cc"));

        ret = mpath::join("aa", "bb/", "cc") == "aa/bb/cc";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::join("aa", "bb/", "cc"):%s == "aa/bb/cc";)"
                     "\n",
                     mpath::join("aa", "bb/", "cc").c_str());

        ret = mpath::parent("./aa/bb") == "aa";
        if (!ret)
            mprintfE(R"(Failed when check: mpath::parent("./aa/bb"):%s == "aa";)"
                     "\n",

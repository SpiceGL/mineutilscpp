...
namespace mstr
{
    ...
    std::vector<std::string> split(const std::string& s, const std::string& sep, bool skip_empty = false, size_t split_times = -1);
    ...
    std::vector<std::string> rsplit(const std::string& s, const std::string& sep, bool skip_empty = false, size_t max_splits = -1);

    /*  实现对指定字符集的正向查找分割功能，以vector形式返回。分割空字符串会返回包含一个空字符串的vector
        @param s：待分割的字符串
        @param sep_chars：分割符集合，不可为空字符串
        @param max_split_times：最大分割次数，-1代表全部分割
        @return 分割结果，至少返回包含一个元素的vector   */
    std::vector<std::string> splitAny(const std::string& s, const std::string& sep_chars, size_t max_splits = -1);

    std::vector<std::string> rsplitAny(const std::string& s, const std::string& sep_chars, size_t max_splits = -1);
...
namespace mstr
{
    ...
    inline std::string _ordinalize(T num)
    {
        ...
        if ((abs_num % 10 == 1) && (abs_num % 100 != 11))
            ...
            return mstr::toStr(num).append("th");
    }
    ...
    inline std::vector<std::string> split(const std::string& s, const std::string& sep, bool skip_empty, size_t max_splits)
    {
        if (s.empty() || max_splits == 0)
            return {s};
        if (sep.empty())
        {
            mprintfE("Param sep is empty!\n");
            return {s};
        }
        ...
        size_t start_pos = 0;
        size_t sep_pos;
        while (now_split_times < max_splits && (sep_pos = s.find(sep.c_str(), start_pos), sep_pos != std::string::npos))
        {
    ...
    inline std::vector<std::string> rsplit(const std::string& s, const std::string& sep, bool skip_empty, size_t max_splits)
    {
        if (s.empty() || max_splits == 0)
            return {s};
        if (sep.empty())
        ...
        std::vector<std::string> strs;
        std::stack<std::string> temp_strs;
        ...
        size_t tmp_sep_pos;
        while (now_split_times < max_splits && sep_pos != 0 && (tmp_sep_pos = s.rfind(sep, find_start_pos), tmp_sep_pos != std::string::npos))
        {
            // 如果分隔符的末位坐标超过了理论反向查找的起始位置，则该次查找应被忽略
            if (tmp_sep_pos + sep.length() - 1 > start_pos)
            {
                if (find_start_pos == 0)
                    break;
                find_start_pos--;
            ...
            sep_pos = tmp_sep_pos;
            size_t substr_len = start_pos - sep_pos + 1 - sep.length();  // 坐标相减再加1代表子串长度
            if (substr_len != 0 || !skip_empty)
            {
                temp_strs.emplace(s.substr(sep_pos + sep.length(), substr_len));
                now_split_times++;
            }
            start_pos = sep_pos - 1;
        ...
        if (sep_pos != 0 || !skip_empty)
            temp_strs.emplace(s.substr(0, sep_pos));
        while (!temp_strs.empty())
        {
            strs.emplace_back(std::move(temp_strs.top()));
            temp_strs.pop();
        }
        return strs;
    }
    ...
    inline std::vector<std::string> splitAny(const std::string& s, const std::string& sep_chars, size_t max_splits)
    {
        if (s.empty() || max_splits == 0)
            return {s};
        if (sep_chars.empty())
        ...
        size_t start_pos = 0;
        size_t sep_pos;
        while (sep_pos = s.find_first_of(sep_chars, start_pos), sep_pos != std::string::npos)
        {
            strs.emplace_back(s.substr(start_pos, sep_pos - start_pos));
            start_pos = sep_pos + 1;
            now_split_times++;
            if (now_split_times >= max_splits)
                break;
        }
        strs.emplace_back(s.substr(start_pos));
        return strs;
    ...
    inline std::vector<std::string> rsplitAny(const std::string& s, const std::string& sep_chars, size_t max_splits)
    {
        if (s.empty() || max_splits == 0)
            return {s};
        if (sep_chars.empty())
        ...
        while (tmp_sep_pos = s.find_last_of(sep_chars, start_pos), tmp_sep_pos != std::string::npos)
        {
            sep_pos = tmp_sep_pos;
            temp_strs.emplace(s.substr(tmp_sep_pos + 1, start_pos - (tmp_sep_pos + 1) + 1));
            start_pos = sep_pos - 1;
            now_split_times++;
            if (now_split_times >= max_splits || sep_pos == 0)
                break;
        }
        temp_strs.emplace(s.substr(0, sep_pos));
        while (!temp_strs.empty())
...
namespace _mstrcheck
{
    inline void ordinalizeTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::ordinalize(1) == "1st" && mstr::ordinalize(2) == "2nd" && mstr::ordinalize(3) == "3rd" && mstr::ordinalize(10) == "10th")"
                     "\n");
        ret0 = (mstr::ordinalize(21) == "21st" && mstr::ordinalize(22) == "22nd" && mstr::ordinalize(-23) == "-23rd");
        if (!ret0)
            mprintfE(R"(Failed when check: mmstr::ordinalize(21) == "21st" && mstr::ordinalize(22) == "22nd" && mstr::ordinalize(-23) == "-23rd")"
                     "\n");
        ret0 = (mstr::ordinalize(11) == "11th" && mstr::ordinalize(12) == "12th" && mstr::ordinalize(-13) == "-13th");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::ordinalize(11) == "11th" && mstr::ordinalize(12) == "12th" && mstr::ordinalize(-13) == "-13th")"
                     "\n");
    }
    inline void toStrTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::toStr(std::vector<int>({ 1, 2, 3 })) == "{1, 2, 3}")"
                     "\n");

        ret0 = (mstr::toStr(std::vector<std::map<int, int>>({{{1, 2}, {3, 4}}})) == "{{1:2, 3:4}}");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::toStr(std::vector<std::map<int, int>>({ {{1, 2}, {3, 4}} })) == "{{1:2, 3:4}}"")"
                     "\n");
        auto tp = std::tuple<std::map<int, int>>({{1, 2}, {3, 4}});
        ret0 = (mstr::toStr(tp) == "{{1:2, 3:4}}");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::toStr(std::tuple<std::map<int, int>>({ {{1, 2}, {3, 4}} })) == "{{1:2, 3:4}}")"
                     "\n");
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: (mstr::toStr(s1) == "hello") && (mstr::toStr(s2) == "hello") && (mstr::toStr(s3) == "hello"))"
                     "\n");
        ret0 = (mstr::toStr(s5) == "hello") && (mstr::toStr(s6) == "hello") && (mstr::toStr(s9) == "hello");
        if (!ret0)
            mprintfE(R"(Failed when check: (mstr::toStr(s5) == "hello") && (mstr::toStr(s6) == "hello") && (mstr::toStr(s9) == "hello"))"
                     "\n");
        ret0 = (mstr::toStr(s10) == "{hello}");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::toStr(s10) == "{hello}")"
                     "\n");

        printf("Author check: mstr::Got toStr(s4) result(0x...):%s, mstr::toStr(s8) result({104, 101, 108, 108, 111}):%s\n", mstr::toStr(s4).c_str(), mstr::toStr(s8).c_str());
        printf("Author check: mstr::Got toStr(s11) result(0x...):%s\n", mstr::toStr(s11).c_str());
    }
    ...
    inline void formatTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::format("I need {} and {}  ", 1, 2) == "I need 1 and 2  ")"
                     "\n");
        ret0 = (mstr::format("I need {}, {} and {}  ", 1, 2) == "I need 1, 2 and {}  ");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::format("I need {}, {} and {}  ", 1, 2) == "I need 1, 2 and {}  "")"
                     "\n");
        ret0 = (mstr::format("I need {}  ", 1, 2) == "I need 1  ");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::format("I need {}  ", 1, 2) == "I need 1  ")"
                     "\n");

        ret0 = (mstr::format("vector: {}", std::vector<std::vector<int>>({{0, 1}, {2, 3}})) == "vector: {{0, 1}, {2, 3}}");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::format("vector: {}", std::vector<std::vector<int>>({ {0, 1}, {2, 3} })) == "vector: {{0, 1}, {2, 3}}")"
                     "\n");
    }
    ...
    inline void splitTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::split("I need {} and {}  ", "{}") == std::vector<std::string>({ "I need ", " and ", "  " }))"
                     "\n");

        ret0 = (mstr::split("I need {} and {}  ", "x") == std::vector<std::string>({"I need {} and {}  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::split("I need {} and {}  ", "x") == std::vector<std::string>({ "I need {} and {}  " }))"
                     "\n");
        ret0 = (mstr::split("I need {} and {}  ", "{}", false, 1) == std::vector<std::string>({"I need ", " and {}  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::split("I need {} and {}  ", "{}", 1) == std::vector<std::string>({ "I need ", " and {}  " }))"
                     "\n");
        ret0 = (mstr::split("aaaaa", "aa") == std::vector<std::string>({"", "", "a"}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::split("aaaaa", "aa") == std::vector<std::string>({ "", "", "a" }))"
                     "\n");

        ret0 = (mstr::rsplit("I need {} and {}  ", "{}") == std::vector<std::string>({"I need ", " and ", "  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplit("I need {} and {}  ", "{}") == std::vector<std::string>({ "I need ", " and ", "  " }))"
                     "\n");

        ret0 = (mstr::rsplit("I need {} and {}  ", "x") == std::vector<std::string>({"I need {} and {}  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplit("I need {} and {}  ", "x") == std::vector<std::string>({ "I need {} and {}  " }))"
                     "\n");
        ret0 = (mstr::rsplit("I need {} and {}  ", "{}", false, 1) == std::vector<std::string>({"I need {} and ", "  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplit("I need {} and {}  ", "{}", 1) == std::vector<std::string>({ "I need {} and ", "  " }))"
                     "\n");
        ret0 = (mstr::rsplit("aaaaa", "aa") == std::vector<std::string>({"a", "", ""}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplit("aaaaa", "aa") == std::vector<std::string>({ "a", "", "" }))"
                     "\n");

        ret0 = (mstr::splitAny("I need {} and {}  ", "{}") == std::vector<std::string>({"I need ", "", " and ", "", "  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::splitAny("I need {} and {}  ", "{}") == std::vector<std::string>({"I need ", "", " and ", "", "  "}))"
                     "\n");
        ret0 = (mstr::splitAny("abcde", "cd") == std::vector<std::string>({"ab", "", "e"}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::splitAny("abcde", "cd") == std::vector<std::string>({"ab", "", "e"}))"
                     "\n");
        ret0 = (mstr::rsplitAny("I need {} and {}  ", "{}") == std::vector<std::string>({"I need ", "", " and ", "", "  "}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplitAny("I need {} and {}  ", "{}") == std::vector<std::string>({"I need ", "", " and ", "", "  "}))"
                     "\n");
        ret0 = (mstr::rsplitAny("abcde", "cd") == std::vector<std::string>({"ab", "", "e"}));
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rsplitAny("abcde", "cd") == std::vector<std::string>({"ab", "", "e"}))"
                     "\n");
    }
    ...
    inline void trimTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::trim(" \n\t 123 \v\r") == "123" && mstr::trim(" \n\t 123") == "123", mstr::trim("123 \v\r") == "123")"
                     "\n");

        ret0 = (mstr::ltrim(" \n\t 123 \v\r") == "123 \v\r" && mstr::ltrim(" \n\t 123") == "123", mstr::ltrim("123 \v\r") == "123 \v\r");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::ltrim(" \n\t 123 \v\r") == "123 \v\r" && mstr::ltrim(" \n\t 123") == "123", mstr::ltrim("123 \v\r") == "123 \v\r")"
                     "\n");

        ret0 = (mstr::rtrim(" \n\t 123 \v\r") == " \n\t 123" && mstr::rtrim(" \n\t 123") == " \n\t 123", mstr::rtrim("123 \v\r") == "123");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rtrim(" \n\t 123 \v\r") == " \n\t 123" && mstr::rtrim(" \n\t 123") == " \n\t 123", mstr::rtrim("123 \v\r") == "123")"
                     "\n");
    }
    ...
    inline void padTest()
    {
        ...
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::lpad("5", 5, '0') == "00005")"
                     "\n");
        ret0 = (mstr::rpad("1.5", 3, '0') == "1.5000");
        if (!ret0)
            mprintfE(R"(Failed when check: mstr::rpad("1.5", 3, '0') == "1.5000")"
                     "\n");
    }

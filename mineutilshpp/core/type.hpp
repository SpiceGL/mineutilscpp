...
namespace mtype
{
    ...
    namespace _mpriv
    {
        ...
        public:
            ...
            static constexpr bool value = (_mpriv::MemFuncCvChecker<MemFn>::is_const_volatile || _mpriv::MemFuncCvChecker<MemFn>::is_only_const && !std::is_volatile<Obj>::value || _mpriv::MemFuncCvChecker<MemFn>::is_only_volatile && !std::is_const<Obj>::value || _mpriv::MemFuncCvChecker<MemFn>::is_no_cv && !std::is_const<Obj>::value && !std::is_volatile<Obj>::value) && (std::is_same<decltype(_mpriv::MemFuncCvChecker<MemFn>::template checkObjType(std::declval<MemFn>())), DecayObj>::value || std::is_base_of<decltype(_mpriv::MemFuncCvChecker<MemFn>::template checkObjType(std::declval<MemFn>())), DecayObj>::value);
        };
        template <class MemFn, class ObjMaybePtr, class... Args>
    ...
    /*  检查Fn和Args...参数是否能使用std::bind绑定，且具有更严格的限制，支持模板SFINAE特性
        ...
        - 仿函数作为Fn，但匹配Args...的operator()为私有或受保护的成员时  */
    template <class Fn, class... Args>
    struct StrictStdBindTraits : public _mpriv::StdBindTraitsHelper<decltype(_mpriv::StdBindTraitsBase<Fn, Args...>::template checkValue<Fn, Args...>(0))::value, Fn, Args...>
    {
    private:
    ...
    namespace _mpriv
    {
        ...
        {
        private:
            template <class U, typename std::enable_if<!_mpriv::AnyCVRefChecker<U>::value && (std::is_arithmetic<U>::value), int>::type = 0>
            static std::true_type check(int);
...
namespace _mtypecheck
{
    ...
    inline void FunctionCheckerTest()
    {
        ...
        printf("Author check! FuncTraits<decltype(_myFunc1)>::RetType(int):%s, ArgsTupleType(std::tuple<int const&, int>):%s, num_args(2):%d.\n", mtype::getTypeName<type0>(), mtype::getTypeName<tuple_type0>(), num_args);
        ...
        printf("Author check! FuncTraits<decltype(&MyClass3::func1)>::ReturnType(int):%s, num_args(1):%d.\n", mtype::getTypeName<type1>(), num_args);
        ...
        printf("Author check! FuncTraits<decltype(&MyClass3::func3)>::ReturnType(int):%s, num_args(1):%d.\n", mtype::getTypeName<type1>(), num_args);
    ...
    {
        static_assert(mtype::StrictStdBindTraits<int, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(_myFunc1), int, float>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(_myFunc1), int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(_myFunc1), int, int, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(_myFunc2), void>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<void, void>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func1), int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func1), std::vector<int>>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func1), const MyClass3*, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func2), const MyClass3*, int>::value == true, "assert failed!");

        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func2), MyClass2*, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::func3), const volatile MyClass3&, int>::value == false, "assert failed!");  // 没有默认可接收volatile类型的拷贝构造函数
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::staticFunc1), MyClass3*, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&MyClass3::staticFunc1), int>::value == true, "assert failed!");

        static_assert(mtype::StrictStdBindTraits<Functor1, short&&>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<std::reference_wrapper<Functor1>, short&&>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<std::reference_wrapper<const Functor1>>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<std::reference_wrapper<const Functor1>, short>::value == false, "assert failed!");

        static_assert(mtype::StrictStdBindTraits<Functor1, int&&>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<Functor1, short>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<volatile Functor1, short>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<const Functor1, short>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&Functor1Son::func), Functor1Son, int>::value == true, "assert failed!");

        static_assert(mtype::StrictStdBindTraits<decltype(&Functor1::func), std::reference_wrapper<Functor1Son>, int>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&Functor1::func), std::reference_wrapper<Functor1Son>&, int>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&Functor1::func), std::reference_wrapper<volatile Functor1Son>, int>::value == false, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&Functor1::funcV), std::reference_wrapper<volatile Functor1Son>, int>::value == true, "assert failed!");

        static_assert(mtype::SameTypesChecker<mtype::StrictStdBindTraits<decltype(constFunc), int*>::ReturnType, const int*>::value, "Check StrictStdBindTraits failed!");
        static_assert(mtype::SameTypesChecker<mtype::StrictStdBindTraits<decltype(constFunc2), MyClass1>::ReturnType, const MyClass1>::value, "Check StrictStdBindTraits failed!");

#if !defined(__GNUC__) || (defined(__GNUC__) && _mgccMinVersion(4, 8, 1))
        static_assert(mtype::StrictStdBindTraits<Functor3, int>::value == false, "assert failed!");
#endif
        static_assert(mtype::StrictStdBindTraits<Functor3, float>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<decltype(&Functor3::func1), Functor3*, int*>::value == true, "assert failed!");
        static_assert(mtype::StrictStdBindTraits<Functor3, int*>::value == true, "assert failed!");

        Functor3 functor3;
        int a = 0;
        std::bind(&Functor3::func1, &functor3, &a)();

        using type0 = mtype::StrictStdBindTraits<decltype(_myFunc1), int, float>::ReturnType;
        printf("Author check! StrictStdBindTraits<decltype(_myFunc1), int, float>::ReturnType(int):%s.\n", mtype::getTypeName<type0>());

        using type1 = mtype::StrictStdBindTraits<decltype(&MyClass3::func2), const MyClass3*, int>::ReturnType;
        printf("Author check! StrictStdBindTraits<decltype(&MyClass3::func2), const MyClass3*, int>::ReturnType(int):%s.\n", mtype::getTypeName<type1>());
    }
